...
impl<'a, B: BitmapSlice> VsockPacket<'a, B> {
    ...
    /// # use virtio_queue::{desc::{split::Descriptor as SplitDescriptor, Descriptor}, Queue, QueueT};
    ...
    /// #
    /// #     let v = vec![
    /// #         Descriptor::from(SplitDescriptor::new(0x5_0000, 0x100, VRING_DESC_F_WRITE as u16, 0)),
    /// #         Descriptor::from(SplitDescriptor::new(0x8_0000, 0x100, VRING_DESC_F_WRITE as u16, 0)),
    /// #     ];
    ...
    /// #
    /// #     let v = vec![
    /// #         Descriptor::from(SplitDescriptor::new(0x5_0000, 0x100, 0, 0)),
    /// #         Descriptor::from(SplitDescriptor::new(0x8_0000, 0x100, 0, 0)),
    /// #     ];
    /// #     let mut chain = vq.build_desc_chain(&v);
    /// #     q
    /// # }
...
mod tests {
    ...
    fn test_from_rx_virtq_chain() {
        ...
        // descriptor.
        let v = vec![Descriptor::from(SplitDescriptor::new(
            0x5_0000,
    ...
    fn test_from_tx_virtq_chain() {
        ...
        let v = vec![
            ...
            Descriptor::from(SplitDescriptor::new(0x5_0000, PKT_HEADER_SIZE as u32, 0, 0)),
        ];
        let mut chain = queue.build_desc_chain(&v).unwrap();
        assert_eq!(
            VsockPacket::from_tx_virtq_chain(&mem, &mut chain, MAX_PKT_BUF_SIZE).unwrap_err(),
        ...
        let v = vec![
            ...
            Descriptor::from(SplitDescriptor::new(0x8_0000, LEN - 1, 0, 0)),
        ];
        let mut chain = queue.build_desc_chain(&v).unwrap();
        assert_eq!(
            VsockPacket::from_tx_virtq_chain(&mem, &mut chain, MAX_PKT_BUF_SIZE).unwrap_err(),
